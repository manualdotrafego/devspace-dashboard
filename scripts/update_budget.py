...
ADSET_ID = os.environ.get('ADSET_ID', '120248610894970581')
ADD_BRL  = float(os.environ.get('ADD_BRL', '40'))
ADD_CENTS = int(ADD_BRL * 100)

print(f"[+] Atualizando conjunto {ADSET_ID} (+R${ADD_BRL:.2f})")
...
current_cents = int(data.get('daily_budget') or data.get('lifetime_budget') or 0)
...
if u.ok and 'success' in u.text:
    print(f"OK: Conjunto atualizado para R${new_cents/100:.2f}/dia")
else:
    print("WARN: Verifique resposta acima")
